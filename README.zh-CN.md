...
- `content.css`：右侧大纲栏样式。
- `icons/`：扩展图标。
- `chatgpt-outline-v0.1.19.zip`：打包后的扩展文件。
...
官方大纲屏蔽逻辑使用了保守的 DOM 特征判断：显式的大纲 / 目录标签、固定或粘性侧边面板、普通布局中贴近右侧或位于主对话阅读列右侧且包含用户追问片段或助手标题片段的窄侧栏，以及 ChatGPT 官方由多个 `Prompt N` 小按钮组成的右侧固定导航 rail。脚本还会接管旧版本注入的同名大纲节点，避免多个本地开发版同时安装时旧内容脚本抢先执行。如果 ChatGPT 再次调整实现，可能需要继续更新这部分规则。
