...
ChatGPT Outline 适合经常让 ChatGPT 写方案、做调研、解释代码、整理长文档的人。它会把助手回复里的标题提取成右侧大纲，默认只显示一列短灰色标记，鼠标悬停时展开为标题卡片，点击任意标题即可跳回对应段落。
...
- 只从助手 / GPT 回复中提取标题，不把用户消息加入大纲。

## 工作方式

扩展会在 `chatgpt.com` 和 `chat.openai.com` 的对话页里扫描助手回复，读取页面已经渲染出来的标题，并按当前回复里最高层级的标题生成大纲：优先 `h1`，没有时用 `h2`，再没有时用 `h3`。
...
- `content.js`：页面注入、标题提取、滚动定位、阅读位置计算、官方大纲屏蔽逻辑。
- `content.css`：右侧大纲栏和悬停卡片样式。
- `icons/`：扩展图标。
- `assets/preview.svg`：README 预览图。
- `chatgpt-outline-v0.1.19.zip`：当前打包后的扩展文件。
